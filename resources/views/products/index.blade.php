                ...
                <div class="p-6">
                    <form method="GET" action="{{ route('products.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <select name="supplier_id" id="supplier_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    ...
                    @if(request('low_stock'))
                        <div class="mt-4">
                            <a href="{{ route('products.index', array_merge(request()->query(), ['low_stock' => '1'])) }}" 
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($products as $product)
                                    <tr class="{{ $product->isLowStock() ? 'bg-red-50' : '' }}">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {{ $product->isLowStock() ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' }}">
                                                {{ $product->stock_quantity }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            <div>Buy: Tsh {{ number_format($product->purchase_price) }}</div>
                                            <div>Sell: Tsh {{ number_format($product->selling_price) }}</div>
                                            @if($product->discount_price)
                                                <div class="text-red-600">Disc: Tsh {{ number_format($product->discount_price) }}</div>
                                            @endif
                                        </td>
